...
TEST(DeletePacketCreator, CreatePacket)
{
    RequestProcessor::SetInstance(NULL);
    Packet *packet = ALLOCATE(Packet);
    EXPECT_EQ(SMB_ERROR, packetCreator->CreatePacket(NULL, DELETE_INIT_REQ, NULL));
    EXPECT_EQ(SMB_ERROR, packetCreator->CreatePacket(packet, DELETE_INIT_REQ, NULL));
    EXPECT_EQ(SMB_ERROR, packetCreator->CreatePacket(packet, DELETE_INIT_RESP, NULL));
    EXPECT_EQ(SMB_ERROR, packetCreator->CreatePacket(packet, DELETE_ERROR_RESP, NULL));
    RequestProcessor::SetInstance(processor);
    bool is_directory;
    EXPECT_EQ(SMB_SUCCESS, packetCreator->CreatePacket(packet, DELETE_INIT_REQ, NULL));
    EXPECT_EQ(SMB_ERROR, packetCreator->CreatePacket(packet, DELETE_INIT_RESP, NULL));
    EXPECT_EQ(SMB_SUCCESS, packetCreator->CreatePacket(packet, DELETE_INIT_RESP, &is_directory));
